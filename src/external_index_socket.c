...
static ExternalIndexResponseError
check_external_index_response(external_index_socket_t *socket_con, char *buffer,
                              int64 size) {
  uint32 hdr;
  if (size < 0) {
...
static void
check_external_index_request_error(external_index_socket_t *socket_con,
                                   int64 bytes_written) {
  if (bytes_written > 0)
    return;
...
static void writeall_or_error(external_index_socket_t *socket_con,
...
create_external_index_session(const char *host, int port, bool secure,
  ...
  external_index_socket_t *socket_con =
      palloc0(sizeof(external_index_socket_t));
  ...
  int64 buf_size = socket_con->read(socket_con, (char *)&init_response,
                                    EXTERNAL_INDEX_INIT_BUFFER_SIZE);

  check_external_index_response_error(socket_con, (char *)init_response,
                                      buf_size);
