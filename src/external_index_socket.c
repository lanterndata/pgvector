...
int64 write_plain(external_index_socket_t *socket_con, const char *buf,
                  size_t size) {
...
/*
 * We will try to create a non-blocking socket and use select syscall with
 * specified timeout After the select will return we will check if socket is
 * writable convert it back to blocking mode and return 0 if so else we will
 * return -1 to indicate that connection attempt was failed. We are using this
 * approach because the process hangs waiting for blocking socket when trying to
 * connect for example to non-routable ip address
 * */
...
static void wait_for_data(external_index_socket_t *socket_con) {
  ...
  while (1) {
    ...
    if (activity < 0) {
      ...
      // If this signals are important they would be handled in
      // CHECK_FOR_INTERRUPTS() If after calling CHECK_FOR_INTERRUPTS() we are
      // still here we can ignore the signal
      if (errno == EINTR)
        continue;
      snprintf((char *)&errstr, EXTERNAL_INDEX_MAX_ERR_SIZE, "%s",
               strerror(errno));
 ...
 *  - read size is less then zero. (this can happen on network errors, or when
 * the server will be closed)
 *  - packet starts with EXTERNAL_INDEX_ERR_MSG bytes. (this will be send from
 * the server indicating that something gone wrong in the server and the
 * following bytes will be error message and will be interpreted as string in
 *    elog(ERROR))
 */
static void
check_external_index_response_status(external_index_socket_t *socket_con,
  ...
  // the server should send err_msg_bytes (uint32) followed by the actual error
  // message we will read and check errors here manually to not get stuck into
  // recursion
  ...
  if (total_bytes_read > 0) {
    err_len_size_read =
        total_bytes_read >= sizeof(uint32) ? sizeof(uint32) : total_bytes_read;
    memcpy(&err_len_bytes, buffer + EXTERNAL_INDEX_MAGIC_MSG_SIZE,
           err_len_size_read);
  ...
  if (total_bytes_read > 0) {
    memcpy(&recv_error,
           buffer + EXTERNAL_INDEX_MAGIC_MSG_SIZE + err_len_size_read,
           total_bytes_read);
  }
  ...
  if (err_len_size_read < sizeof(uint32)) {
    bytes_read =
        socket_con->read(socket_con, (char *)&err_len_bytes + err_len_size_read,
                         sizeof(uint32) - err_len_size_read);
  ...
  while (total_bytes_read < err_msg_size) {
    bytes_read =
        socket_con->read(socket_con, (char *)&recv_error + total_bytes_read,
                         err_msg_size - total_bytes_read);
...
static void
check_external_index_request_status(external_index_socket_t *socket_con,
                                    int64 bytes_written) {
  CHECK_FOR_INTERRUPTS();
...
static void external_index_write_all(external_index_socket_t *socket_con,
                                     const char *buf, uint32 len, int flags) {
  int32 total = 0;
...
uint64 external_index_read_all(external_index_socket_t *socket_con,
                               char *result_buf, uint64 size) {
  int64 bytes_read;
...
void create_external_index_session(const char *host, int port, bool secure,
                                   const usearch_init_options_t *params,
                                   const HnswBuildState *buildstate,
                                   uint32 estimated_row_count) {
  int client_fd, status;
  char
      init_buf[sizeof(external_index_params_t) + EXTERNAL_INDEX_MAGIC_MSG_SIZE];
  char port_str[6];
...
#else
  if (secure) {
    elog(ERROR, "Can not use secure connection as Postgres is not compiled "
                "with openssl support. Set "
  ...
  if ((status = connect_with_timeout(client_fd, serv_addr->ai_addr,
                                     serv_addr->ai_addrlen,
                                     EXTERNAL_INDEX_SOCKET_TIMEOUT)) < 0) {
  ...
  bytes_read = buildstate->external_socket->read(
      buildstate->external_socket, (char *)&init_response, sizeof(uint32));
  check_external_index_response_status(buildstate->external_socket,
                                       (char *)init_response, bytes_read);
  ...
  if (protocol_version != EXTERNAL_INDEX_PROTOCOL_VERSION) {
    elog(ERROR,
         "external index protocol version mismatch - client version: %u, "
         "server version: %u",
         EXTERNAL_INDEX_PROTOCOL_VERSION, protocol_version);
  }
  // check server type
  bytes_read = buildstate->external_socket->read(
      buildstate->external_socket, (char *)&init_response, sizeof(uint32));
  ...
  if (server_type == EXTERNAL_INDEX_ROUTER_SERVER_TYPE) {
    ...
    elog(INFO, "receiving new server address from router... (this may take up "
               "to 10m)");
    memcpy(init_buf, &get_server_msg, sizeof(uint32));
    external_index_write_all(buildstate->external_socket, init_buf,
                             sizeof(uint32), 0);

    // wait for data to be available for read and also check for interrupts each
    // 1s
    ...
    external_index_read_all(buildstate->external_socket, (char *)&address,
                            address_length);
    ...
    return create_external_index_session(address, port_number, (bool)is_secure,
                                         params, buildstate,
                                         estimated_row_count);
  }
  ...
  external_index_params_t index_params = {
      ...
      .element_bits = buildstate->scalar_bits,
  };

  uint32 hdr_msg = EXTERNAL_INDEX_INIT_MSG;
  memcpy(init_buf, &hdr_msg, EXTERNAL_INDEX_MAGIC_MSG_SIZE);
  memcpy(init_buf + EXTERNAL_INDEX_MAGIC_MSG_SIZE, &index_params,
         sizeof(external_index_params_t));

  external_index_write_all(
      buildstate->external_socket, init_buf,
      sizeof(external_index_params_t) + EXTERNAL_INDEX_MAGIC_MSG_SIZE, 0);

  bytes_read = buildstate->external_socket->read(
      buildstate->external_socket, (char *)&init_response,
      EXTERNAL_INDEX_INIT_BUFFER_SIZE);
...
void external_index_receive_metadata(external_index_socket_t *socket_con,
  ...
  external_index_write_all(socket_con, (char *)&end_msg,
                           EXTERNAL_INDEX_MAGIC_MSG_SIZE, 0);
